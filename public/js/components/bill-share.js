...
const BillShare = {
  ...
  async shareToWhatsApp(sale, settings, phone) {
    ...
    // Always jump straight into the WhatsApp chat for this number - this is
    // what "open instantly" means: no intermediate OS share-sheet picker
    // with a dozen unrelated apps in it. The PDF is downloaded first so
    // it's ready to attach.
    //
    // There is a real technical limit here that's worth being upfront
    // about: no website can hand a file directly into a WhatsApp chat.
    // wa.me only opens the chat with a pre-typed message - the file still
    // has to be attached by hand (tap the paperclip, pick the just-downloaded
    // file). That extra tap is unavoidable; what this fixes is skipping the
    // generic share sheet and landing you in the right chat immediately.
