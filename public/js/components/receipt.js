...
const Receipt = {
  ...
  buildHtml(sale, settings) {
    ...
    // Configurable print-safe sizing (fixes text getting clipped at the left
    // /right edge on physical thermal printers, which almost always have a
    // printable area narrower than the full paper roll width) - see the
    // "Advanced Size Adjustment" fields in Settings.
    const marginMm = settings.receiptMarginMm ?? 3;
    const safeWidthPct = Math.min(100, Math.max(70, settings.receiptSafeWidthPct || 94));
    const contentPx = isA4 ? '100%' : Math.round(paperPx * (safeWidthPct / 100)) + 'px';
    const lineSpacing = settings.receiptLineSpacing || 1.15;

    return `<!DOCTYPE html><html><head><meta charset="UTF-8">
<title>Receipt</title>
<style>
  @page { size:${isA4?'A4':`${paperPx}px auto`}; margin:${isA4?'12mm':marginMm+'mm'}; }
  *{box-sizing:border-box;margin:0;padding:0}
  html,body{background:#fff}
  body{font-family:Arial,'Helvetica Neue',Helvetica,sans-serif;font-size:${base}px;font-weight:${fw};line-height:${lineSpacing};
       width:${contentPx};max-width:${isA4?'680px':'none'};
  ...
  @media print{
    body{padding:0;margin:0 auto;-webkit-print-color-adjust:exact;print-color-adjust:exact}
    * {-webkit-print-color-adjust:exact !important;print-color-adjust:exact !important;color:#000;-webkit-text-stroke:0.12px #000}
  }
</style></head><body>
...
<div class="center" style="margin:8px 0">
  ...
  <div style="font-size:${base-2}px;color:#777;margin-top:2px">For tax verification only - scanning shows raw data, not a receipt.</div>
</div>
<hr class="divider">` : ''}
${(settings.enableDigitalReceipt !== false && sale.invoiceNo) ? `
<div class="center" style="margin:8px 0">
