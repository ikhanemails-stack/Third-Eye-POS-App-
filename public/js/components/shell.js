...
const Shell = {
  ...
  render(activePath, contentHtml) {
    ...
    // Driver accounts only get the Delivery module - no sections, no other
    // links. Everything else (inventory, accounting, staff, etc.) is hidden
    // here AND blocked server-side (see restrictDriver in server/helpers.js).
    const navHtml = user.role === 'driver'
      ? `<a href="#/delivery" class="nav-item active">
          <span style="width:18px;height:18px;display:flex">${Icon.delivery}</span>
          <span>Delivery</span>
        </a>`
            ...
            <div class="topbar-right">
              ...
              <div class="topbar-user">
                ...
                <div>
                  ...
                  <div style="color:var(--text-muted);font-size:0.74rem">${user.role === 'admin' ? 'Administrator' : user.role === 'driver' ? 'Driver' : 'Cashier'}</div>
                </div>
              </div>
