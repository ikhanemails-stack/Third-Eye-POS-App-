...
const App = {
  ...
  registerRoutes() {
    ...
    Router.register('/quotations', () => QuotationsScreen.render());
    ...
    // Driver accounts only ever see the Delivery screen - redirect every
    // other route straight back to it (server-side, restrictDriver in
    // helpers.js blocks the underlying API calls too, so this isn't just
    // cosmetic).
    if (this.user && this.user.role === 'driver') {
      Object.keys(Router.routes).forEach(path => {
        if (path !== '/delivery') {
          Router.routes[path] = () => Router.navigate('/delivery');
        }
      });
    }
  }
