...
const AccountingScreen = {
  ...
  openExpenseModal(expense) {
    ...
    document.getElementById('expense-form').addEventListener('submit', async (e) => {
      e.preventDefault();
      const payload = {
        categoryId: Number(document.getElementById('e-category').value) || null,
        ...
        ` : `
          ...
          <form id="open-session-form" style="display:flex;gap:10px;align-items:flex-end">
            <div class="form-group" style="margin-bottom:0;flex:1">
              <label class="form-label">Opening Float (${escapeHtml(App.settings.currency || 'BHD')})</label>
  ...
  openCloseSessionModal() {
    Modal.open('Close Cash Drawer', `
      <form id="close-session-form">
        <div class="form-group">
          <label class="form-label">Counted Cash in Drawer (${escapeHtml(App.settings.currency || 'BHD')})</label>
