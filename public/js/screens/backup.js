...
const BackupScreen = {
  ...
  buildHtml() {
    ...
    <div class="backup-grid">
      ...
      <div class="backup-card">
        ...
        <p class="backup-card-desc">Download everything — products (including photos), sales, purchases, quotations, deliveries, customers, suppliers, employees and settings — as a single backup file.</p>
  ...
  async handleBackupFile(event) {
    ...
    try {
      ...
      const products = Array.isArray(backup.tables.products) ? backup.tables.products : [];
      const photoCount = products.filter(p => p && p.photo).length;
      status.innerHTML = `
        <div style="background:#fef3c7;border:1px solid #f59e0b;border-radius:10px;padding:16px;margin-top:8px">
          ...
          <div style="font-size:13px;margin-bottom:4px">Total records: <strong>${total}</strong></div>
          <div style="font-size:13px;margin-bottom:12px">Products with photos: <strong>${photoCount} of ${products.length}</strong></div>
