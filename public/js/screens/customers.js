...
const CustomersScreen = {
  ...
  renderCustomersTab() {
    ...
    const filtered = term ? this.customers.filter(c =>
    ...
    el.innerHTML = `
      <div class="toolbar-row">
        <input class="form-input" id="cust-search" placeholder="Search by name, phone or address..." value="${escapeHtml(this.searchTerm)}" style="max-width:280px">
      ...
      <div class="table-wrap">
        <table>
          ...
          <tbody>
            ${filtered.length === 0 ? `<tr><td colspan="7"><div class="empty-state"><p>${this.customers.length === 0 ? 'No customers yet.' : 'No customers match your search.'}</p></div></td></tr>` : filtered.map(c => `
              <tr>
                ...
                <td style="text-align:right;white-space:nowrap">
                  ...
                  ${c.phone ? `<button class="row-action row-action-adjust msg-cust-btn" data-id="${c.id}" title="Message on WhatsApp">${Icon.whatsapp}</button>` : ''}
    ...
    document.getElementById('cust-search').addEventListener('input', (e) => {
      ...
      this._searchTimer = setTimeout(() => {
        ...
        const newInput = document.getElementById('cust-search');
    ...
    document.querySelectorAll('.msg-cust-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const customer = this.customers.find(c => c.id === Number(btn.dataset.id));
        if (!customer || !customer.phone) return;
        const digits = String(customer.phone).replace(/[^\d]/g, '');
        const withCode = digits.length === 8 ? `973${digits}` : digits;
        const text = `Hi ${customer.name}, this is ${App.settings.shopName || 'our store'}.`;
          ...
          <tbody>
            ${this.coupons.length === 0 ? `<tr><td colspan="6"><div class="empty-state"><p>No coupons created yet.</p></div></td></tr>` : this.coupons.map(c => `
              <tr>
                ...
                <td>${c.discountType === 'percent' ? `${c.discountValue}% off` : `${c.discountValue.toFixed(3)} ${App.settings.currency || 'BHD'} off`}</td>
  ...
  openCouponModal(coupon) {
        ...
        <div class="form-row">
          <div class="form-group">
            ...
            <select class="form-select" id="coupon-type">
              ...
              <option value="fixed" ${coupon?.discountType === 'fixed' ? 'selected' : ''}>Fixed Amount (${escapeHtml(App.settings.currency || 'BHD')})</option>
            </select>
          </div>
          <div class="form-group">
    ...
    document.getElementById('coupon-form').addEventListener('submit', async (e) => {
      ...
      const payload = {
        ...
        maxUses: Number(document.getElementById('coupon-max-uses').value) || null
      };
      if (isEdit) {
