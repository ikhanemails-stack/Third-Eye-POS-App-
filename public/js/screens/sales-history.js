...
const SalesHistoryScreen = {
  ...
  renderScreen() {
              ...
              <tr>
                ...
                <td style="text-align:right;white-space:nowrap">
                  <div class="row-actions-group">
                    ...
                    <button class="row-action row-action-adjust invoice-sale-btn" data-id="${s.id}" title="Print professional A4 tax invoice">${Icon.box}</button>
                    <button class="row-action row-action-adjust whatsapp-invoice-sale-btn" data-id="${s.id}" title="Send A4 Invoice on WhatsApp">${Icon.whatsapp}</button>
    ...
    document.querySelectorAll('.invoice-sale-btn').forEach(btn => {
      btn.addEventListener('click', () => this.printInvoice(Number(btn.dataset.id)));
    });
    document.querySelectorAll('.whatsapp-invoice-sale-btn').forEach(btn => {
      btn.addEventListener('click', () => this.shareInvoice(Number(btn.dataset.id)));
    });
  },
  ...
  async printInvoice(id) {
    try {
      ...
      DocPrint.printInvoice(sale, App.settings);
  ...
  async shareInvoice(id) {
    try {
      const sale = await Api.get(`/sales/${id}`);
      DocShare.openShareModal('invoice', sale, App.settings);
    } catch (err) { Toast.error(err.message); }
  },
    ...
    document.getElementById('transfer-form').addEventListener('submit', async (e) => {
      ...
      try {
        await Api.post(`/sales/${sale.id}/transfer-to-delivery`, {
          ...
          address,
          driverId: Number(document.getElementById('t-driver').value) || null,
