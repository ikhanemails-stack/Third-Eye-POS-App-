...
const SettingsScreen = {
  ...
  renderScreen() {
        ...
        <div class="card-flat" style="margin-bottom:20px">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
            <div>
              ...
              <div class="form-group">
                <label class="form-label" style="display:flex;align-items:center;gap:8px">
                  <input type="checkbox" id="s-enableDigitalReceipt" ${s.enableDigitalReceipt !== false ? 'checked' : ''} style="width:auto" ${!isAdmin ? 'disabled' : ''} onchange="SettingsScreen.updateLivePreview()">
                  Show "Scan to View Receipt Online" QR code
                </label>
                <div class="form-hint">Turn this off to save paper - this QR block (separate from the Saudi ZATCA tax QR, which is controlled automatically by your country setting) isn't required in every country. Bahrain, for example, doesn't need it.</div>
              </div>

              <div class="form-group" style="border-top:1px solid var(--border);padding-top:14px;margin-top:6px">
                <label class="form-label">Advanced Size Adjustment</label>
                <div class="form-hint" style="margin-bottom:10px">
                  If text is getting cut off on the left/right edges of your physical printout, your printer's actual printable area is narrower than the paper roll (most 80mm rolls only print ~72mm). Drag "Content Width" down until the preview edges match your printer, and increase margin if needed. The preview on the right updates instantly as you drag.
                </div>
                <div class="size-slider-group">
                  <div class="size-slider-row">
                    <div class="size-slider-label">
                      <span>Content Width</span>
                      <span class="size-slider-value" id="s-receiptSafeWidthPct-val">${s.receiptSafeWidthPct || 94}%</span>
                    </div>
                    <input class="size-slider" id="s-receiptSafeWidthPct" type="range" min="70" max="100" step="1" value="${s.receiptSafeWidthPct || 94}" ${!isAdmin ? 'disabled' : ''}>
                  </div>
                  <div class="size-slider-row">
                    <div class="size-slider-label">
                      <span>Page Margin</span>
                      <span class="size-slider-value" id="s-receiptMarginMm-val">${s.receiptMarginMm ?? 3}mm</span>
                    </div>
                    <input class="size-slider" id="s-receiptMarginMm" type="range" min="0" max="15" step="0.5" value="${s.receiptMarginMm ?? 3}" ${!isAdmin ? 'disabled' : ''}>
                  </div>
                  <div class="size-slider-row">
                    <div class="size-slider-label">
                      <span>Line Spacing</span>
                      <span class="size-slider-value" id="s-receiptLineSpacing-val">${s.receiptLineSpacing || 1.15}x</span>
                    </div>
                    <input class="size-slider" id="s-receiptLineSpacing" type="range" min="1" max="2" step="0.05" value="${s.receiptLineSpacing || 1.15}" ${!isAdmin ? 'disabled' : ''}>
                  </div>
                </div>
              </div>
    ...
    [
      ['s-receiptSafeWidthPct', v => `${v}%`],
      ['s-receiptMarginMm', v => `${v}mm`],
      ['s-receiptLineSpacing', v => `${v}x`]
    ].forEach(([id, formatVal]) => {
      const slider = document.getElementById(id);
      const readout = document.getElementById(`${id}-val`);
      if (!slider) return;
      slider.addEventListener('input', () => {
        if (readout) readout.textContent = formatVal(slider.value);
        this.updateLivePreview();
      });
    });
  ...
  getDraftSettings() {
    ...
    return {
      ...
      enableDigitalReceipt: document.getElementById('s-enableDigitalReceipt')?.checked,
      receiptSafeWidthPct: Number(document.getElementById('s-receiptSafeWidthPct')?.value) || 94,
      receiptMarginMm: Number(document.getElementById('s-receiptMarginMm')?.value ?? 3),
      receiptLineSpacing: Number(document.getElementById('s-receiptLineSpacing')?.value) || 1.15,
