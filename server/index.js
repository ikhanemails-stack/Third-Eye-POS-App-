...
const { requireLicense, restrictDriver } = require('./helpers');
...
// A backup file from a shop with a lot of product photos (stored as
// inline base64 images) can get large - if one ever exceeds the limit
// above, this turns Express's default HTML error page into a clear JSON
// message the restore screen can actually show, instead of a confusing
// "Restore failed, try again" with no explanation.
app.use((err, req, res, next) => {
  if (err && err.type === 'entity.too.large') {
    return res.status(413).json({ error: 'This backup file is too large to upload in one request. Contact support if this happens - shops with a very large number of product photos may need a different backup approach.' });
  }
  next(err);
});
app.use(session({
...
// Driver accounts can only reach the Delivery module (see helpers.js).
app.use('/api', restrictDriver);
...
app.use('/api', requireLicense, require('./routes/quotations'));
