...
router.post('/returns', requireLogin, (req, res) => {
  ...
  if (productId) {
    ...
    // Once every unit of an expiring/expired batch has been returned to the
    // supplier, clear the product's expiryDate so it drops off the
    // "Expiring Items" list (which is computed live from expiryDate) and
    // only remains visible in Return History below. If stock still remains
    // (a partial return), keep the expiry date so the rest keeps tracking.
    const clearExpiry = newStock <= 0 && ['expired', 'near_expiry'].includes(reason);
