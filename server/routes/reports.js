...
// Accepts either a plain "YYYY-MM-DD" date or a full ISO timestamp and
// returns a Date. For a plain date, `endOfDay` controls whether it's
// anchored to the start (00:00:00.000) or end (23:59:59.999) of that day -
// a full timestamp is used exactly as given since it already carries its
// own time. (Blindly appending a time string onto an already-full ISO
// timestamp - a bug in an earlier fix - produced an invalid date and made
// every report endpoint throw, showing "Could not load report.")
...
router.get('/reports/quotations', requireLogin, (req, res) => {
  const { fromDate, toDate } = dateRange(req);
  const { status } = req.query;

  let quotes = db.all('quotations').filter(q =>
    new Date(q.createdAt) >= fromDate && new Date(q.createdAt) <= toDate
  );
  ...
  const totalQuotedValue = roundMoney(quotes.reduce((sum, q) => sum + q.total, 0), 3);
  const convertedValue = roundMoney(converted.reduce((sum, q) => sum + q.total, 0), 3);
  const conversionRate = quotes.length ? Math.round((converted.length / quotes.length) * 100) : 0;

  const byStatus = { draft: 0, sent: 0, converted: 0, rejected: 0, expired: 0 };
...
router.get('/reports/purchases', requireLogin, (req, res) => {
  const { fromDate, toDate } = dateRange(req);
  const { supplierId } = req.query;

  let purchases = db.all('purchases').filter(p =>
    new Date(p.createdAt) >= fromDate && new Date(p.createdAt) <= toDate
  );
  if (supplierId) purchases = purchases.filter(p => p.supplierId === Number(supplierId));
  ...
  const totalSpend = roundMoney(purchases.reduce((sum, p) => sum + p.total, 0), 3);

  const bySupplier = {};
  purchases.forEach(p => {
    const name = supplierName(p.supplierId);
    bySupplier[name] = roundMoney((bySupplier[name] || 0) + p.total, 3);
  });

  // Top products by quantity/value purchased in the period.
  const purchaseIds = new Set(purchases.map(p => p.id));
  const allItems = db.all('purchase_items').filter(i => purchaseIds.has(i.purchaseId));
  const productTotals = {};
  allItems.forEach(i => {
    if (!productTotals[i.productName]) productTotals[i.productName] = { quantity: 0, value: 0 };
    productTotals[i.productName].quantity += i.quantity;
    productTotals[i.productName].value = roundMoney(productTotals[i.productName].value + i.lineTotal, 3);
  });
  ...
  res.json({
    ...
    bySupplier,
    topProducts,
    list: purchases.map(p => ({ ...p, supplierName: supplierName(p.supplierId) }))
  });
});
